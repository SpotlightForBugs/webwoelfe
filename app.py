...
def handle_audio_fertig(data):
    ...
    if raum.aktuelle_phase == gemeldete_phase:
        ...
        if gemeldete_phase in AUTOMATISCHE_PHASEN:
            ...
            if rolle:
                if game_logic.alle_haben_gewaehlt(
                    raum, gemeldete_phase, rolle.info.name
                ):
                    log_ts(
                        f"[Audio] Smart Advance: Keine offenen Aktionen für {gemeldete_phase} (Rolle: {rolle.info.name})"
                    )
                    should_advance = True

...
class Role(ABC):
    ...
    def execute_action(
        ...
        normalized_action = (
            normalize_action_type(action_type) if action_type else "skip"
        )
        ...
        for button in ui_def.buttons:
            ...
            if (
                ...
                or btn_action_type == normalized_action
                or btn_action_type == original_action
            ):
                # Found matching button - route to on_nacht_aktion with the action
                ziel = targets[0] if len(targets) == 1 else None
                try:
                    result = self.on_nacht_aktion(
                    ...
                    try:
                        ...
                        logger.error(
                            f"on_nacht_aktion failed for button {btn_action_type}: {e}"
                        )
                        return AktionsErgebnis(
                            erfolg=False,

...
export default class Village3DPlayCanvas {
  ...
  createCampfire() {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      stone.setLocalScale(0.4 + Math.random() * 0.2, 0.3, 0.4 + Math.random() * 0.2);
    ...
    for (let i = 0; i < 5; i++) {
      ...
      log.setLocalEulerAngles(90 + Math.random() * 10, i * 72, Math.random() * 20);
  ...
  createTree(x, z, height) {
    ...
    trunk.setLocalScale(0.4 + Math.random() * 0.2, height, 0.4 + Math.random() * 0.2);
    ...
    for (let i = 0; i < 3; i++) {
      ...
      leavesMat.diffuse = new pc.Color(0.05 + Math.random() * 0.05, 0.15 + Math.random() * 0.1, 0.05);
      leavesMat.update();
  ...
  createDetailedPlayer(player, x, z, angle) {
    ...
    // Get role color
    const roleColor = this.roleColors[player.rolle] || this.roleColors['default'];
  ...
  pick(x, y) {
    const from = this.camera.camera.screenToWorld(x, y, this.camera.camera.nearClip);
    const to = this.camera.camera.screenToWorld(x, y, this.camera.camera.farClip);
  ...
  highlightPlayer(playerId, color) {
    ...
    const mat = new pc.StandardMaterial();
    mat.emissive = color === 'red' ? new pc.Color(1, 0, 0) : new pc.Color(0, 1, 0);
    mat.opacity = 0.6;
    mat.blendType = pc.BLEND_ADDITIVE;
    mat.update();
  ...
  showHint(playerId, type) {
    ...
    const mat = new pc.StandardMaterial();
    mat.emissive = type === 'danger' ? new pc.Color(1, 0, 0) : new pc.Color(1, 1, 0);
    mat.opacity = 0.8;
    mat.blendType = pc.BLEND_ADDITIVE;
    mat.update();
    ...
    const updateHint = (dt) => {
      ...
      hint.setLocalScale(0.5 + Math.sin(time * 5) * 0.1, 0.5 + Math.sin(time * 5) * 0.1, 0.5 + Math.sin(time * 5) * 0.1);

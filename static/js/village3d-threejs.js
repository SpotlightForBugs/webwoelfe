...
class Village3D {
  ...
  createCampfire() {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      flame.position.set(Math.cos(angle) * 0.2, 0.3, Math.sin(angle) * 0.2);
  ...
  createHouse(scale = 1.0) {
    ...
    [
      ...
      // Window frame
      const frameOuterGeom = new THREE.BoxGeometry(
        0.4 * scale,
        0.4 * scale,
        0.03,
      );
  ...
  animate() {
    ...
    this.fireParticles.forEach((flame, i) => {
      flame.mesh.position.y =
        flame.baseY + Math.sin(time * 8 + flame.phase) * 0.1;
      flame.mesh.scale.y = 1 + Math.sin(time * 10 + flame.phase) * 0.2;
      flame.mesh.material.opacity =
        0.7 + Math.sin(time * 12 + flame.phase) * 0.3;
    });

    // Fire light flicker
    this.fireLight.intensity =
      2 + Math.sin(time * 8) * 0.3 + Math.sin(time * 13) * 0.2;

    // Star twinkle
    if (this.isNight) {
      this.stars.forEach((star) => {
        star.mesh.material.opacity =
          0.5 + Math.sin(time * 2 + star.phase) * 0.3;
      });
    }

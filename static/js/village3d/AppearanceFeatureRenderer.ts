...
export class AppearanceFeatureRenderer {
  ...
  applyAppearanceFeatures(
    ...
    console.log(
      `[Village3D] Applying ${appearanceFeatures.length} appearance features for ${playerData.rolle}`,
    );
    ...
    appearanceFeatures.forEach((feature, index) => {
      try {
        ...
        console.error(
          `[Village3D] Failed to apply appearance feature:`,
          error,
          feature,
        );
      }
  ...
  private renderFeature(
    ...
    const featureEntity = new pc.Entity(
      `Feature-${feature.feature_type}-${index}`,
    );
    ...
    console.log(
      `[Village3D] Added ${feature.feature_type} feature: ${feature.description || "no description"}`,
    );
  }
  ...
  private createMaterial(
    ...
    const emissiveColor =
      feature.emissive && (feature.emissive_intensity ?? 0) > 0
        ? new pc.Color(

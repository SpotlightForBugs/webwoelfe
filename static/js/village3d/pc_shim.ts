...
export class CameraComponent {
  ...
  constructor(
    ...
    opts: { farClip?: number; clearColor?: unknown } = {},
  ) {
    const THREE = getTHREE();
    const aspect =
...
export class Application extends Emitter {
  ...
  constructor(canvas: HTMLCanvasElement, _opts: any) {
    ...
    (this.renderer as any).outputColorSpace =
      (THREE as any).SRGBColorSpace ?? (this.renderer as any).outputColorSpace;
    (this.renderer as any).toneMapping =
    ...
    this.renderer.shadowMap.type =
      (THREE as any).PCFSoftShadowMap ?? this.renderer.shadowMap.type;
  ...
  start(): void {
    ...
    const tick = (now: number) => {
      ...
      if (camera) {
        const cc = (camera as any).userData?.__pc_clearColor as
          | THREE_NS.Color
          | undefined;

  ...
  _loadGLB(path) {
    if (this._glbCache.has(path)) {
      ...
      return this._glbCache.get(path).then((original) => {
        ...
        const clone = original.meshes[0].clone(
          original.meshes[0].name + "_clone",
          null,
        );
        if (clone) {
          // Clone children with materials
          for (let i = 1; i < original.meshes.length; i++) {
            const childClone = original.meshes[i].clone(
              original.meshes[i].name + "_c",
              clone,
            );
    ...
    const promise = BABYLON.SceneLoader.ImportMeshAsync(
      "",
      "",
      path,
      this._scene,
    ...
    return promise.then((result) => {
      ...
      const root = result.meshes[0].clone(
        result.meshes[0].name + "_first",
        null,
      );
      if (root) {
        for (let i = 1; i < result.meshes.length; i++) {
          const cc = result.meshes[i].clone(result.meshes[i].name + "_f", root);
          if (cc && result.meshes[i].material)
            cc.material = result.meshes[i].material;
        }

...
test.describe('Game 1: Complete Village Victory Game', () => {
  ...
  test('5 players complete a full game where village wins', async ({ browser }) => {
    ...
    for (const player of players) {
      ...
      for (const name of playerNames) {
        await expect(player.page.locator(`text=${name}`)).toBeVisible({ timeout: 10000 });
      }
    }
    ...
    while (!gameEnded && roundCount < maxRounds) {
      ...
      if (currentPhase.includes('werwolf')) {
        ...
        const livingVillagers = villagers.filter(v => v.role && !v.role.includes('werwolf'));
        ...
        if (seer) {
          try {
            ...
            if (werewolves.length > 0) {
              logAction(seer.name, 'examine', werewolves[0].name, 'checking for werewolf');
        ...
        const voteTarget = werewolves.length > 0 ? werewolves[0].name : players[1].name;

...
test.describe('Game 2: Complete Werewolf Victory Game', () => {
  ...
  test('6 players complete a full game where werewolves win', async ({ browser }) => {
    const playerNames = ['Friedrich', 'Greta', 'Hans', 'Inge', 'Johann', 'Klara'];
    ...
    const startButton = players[0].page.locator('#start-btn, button:has-text("Spiel starten"):not([disabled])');
    ...
    while (!gameEnded && roundCount < maxRounds) {
      ...
      if (currentPhase.includes('werwolf')) {
        ...
        if (seer && !seer.role?.includes('werwolf')) {
          ...
          const livingVillagers = villagers.filter(v => !werewolves.includes(v));
        ...
        if (seer) {
          try {
            ...
            if (randomPlayer.name !== seer.name) {
              ...
              await delayAction();
              const seeBtn = seer.page.locator('button:has-text("Sehen"), button:has-text("Bestaetigen")');
        ...
        const voteTarget = villagers.length > 0 ? villagers[0].name : players[1].name;
        
        for (const player of players) {
          try {
            const playerCard = player.page.locator(`.spieler-card:not(.tot)`).first();
...
test.describe('Game 2B: Large 8-Player Game', () => {
  ...
  test('8 players complete a full game', async ({ browser }) => {
    const playerNames = ['Ludwig', 'Marie', 'Norbert', 'Olga', 'Paul', 'Rita', 'Stefan', 'Tina'];

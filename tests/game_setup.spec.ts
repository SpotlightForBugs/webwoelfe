...
const PLAYER_NAMES = [
    ...
    'Heinrich', 'Elisabeth', 'Otto', 'Margarethe', 'Wilhelm', 'Helene', 'Karl', 'Emma',
    'Ludwig', 'Charlotte', 'Franz', 'Katharina', 'Johann', 'Frieda', 'Anton', 'Gertrude',
    'Maximilian', 'Therese', 'Rudolf', 'Johanna', 'Hermann', 'Luise'
];
...
function calculateWindowLayout(index: number, total: number): { x: number; y: number; width: number; height: number } {
...
test.describe('Game Setup - Auto-Create and Stay Open', () => {
    test(`Setup game with ${PLAYER_COUNT} players`, async () => {
        ...
        try {
            ...
            await erzaehlerPage.evaluate(({ x, y }) => {
                window.moveTo(x, y);
            ...
            // Raum erstellen
            const createBtn = erzaehlerPage.locator('button:has-text("Spiel erstellen")');
            ...
            if (urlMatch) {
                ...
                const codeElement = await erzaehlerPage.locator('.room-code, [class*="code"], h2').first();

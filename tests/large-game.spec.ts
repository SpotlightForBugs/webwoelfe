...
test.describe("Large Game Balance Tests", () => {
  ...
  test("API accepts 500+ player count for room creation", async ({
    browser,
  }) => {
    resetWindowPositions();
    ...
    await expect(creator.page.locator("text=500")).toBeVisible({
      timeout: 5000,
    });
  ...
  test("API accepts 1000 player count for room creation", async ({
    browser,
  }) => {
    resetWindowPositions();
  ...
  test("role balance API returns valid distribution for 500 players", async ({
    request,
  }) => {
  ...
  test("role distribution page loads with large numbers", async ({
    browser,
  }) => {
    resetWindowPositions();
  ...
  test("verify balance statistics via Python API", async ({ request }) => {
    ...
    logEvent(
      "INFO",
      "Run: python -m pytest tests/test_large_game_balance.py -v",
    );
...
test.describe("Balance Verification - Mathematical", () => {
  ...
  test("wolf ratio is balanced for various player counts", async ({
    browser,
  }) => {
    const context = await browser.newContext();
    const page = await context.newPage();
    ...
    for (const count of playerCounts) {
      ...
      if (count <= 10) {
        ...
        expectedMaxWolves = Math.floor(count * 0.2);
      }

      console.log(
        `Players: ${count}, Expected wolves: ${expectedMinWolves}-${expectedMaxWolves}`,
      );
...
test.describe("Large Game Room Configuration", () => {
  ...
  test("group mode also supports large player counts", async ({ browser }) => {
    ...
    logEvent(
      "SUCCESS",
      `Group mode room created for 500 players`,
      `Code: ${roomCode}`,
    );

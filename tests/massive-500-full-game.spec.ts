...
async function getHeadfulPage(): Promise<Page> {
  if (!headfulPage) {
    ...
    logEvent('BROWSER', 'Single headful browser window started for observation');
  }
  return headfulPage!;
}
...
async function createMassPlayers(
  ...
  logEvent('MASS_JOIN', `Creating ${count} players: ${HEADLESS_COUNT} headless + ${HEADFUL_COUNT} headful (1 window)...`);
  ...
  logEvent('SUMMARY', `${headlessPlayers} headless + ${headfulPlayers} headful (1 visible window) = ${players.length} total`);
  ...
  const rolePromises = players.map(async (player) => {
    try {
      ...
      if (roleLower.includes('werwolf') && !roleLower.includes('weiß')) {
        player.team = 'wolf';
      } else if (roleLower.includes('vampir') || roleLower.includes('zombie') || 
                 roleLower.includes('flöten') || roleLower.includes('engel') ||
 ...
 */
async function getGameStatus(player: MassPlayer, allPlayers: MassPlayer[]): Promise<GameStatus> {
  ...
  const wolfCount = allPlayers.filter(p => p.isAlive && p.team === 'wolf').length;
  const villageCount = allPlayers.filter(p => p.isAlive && p.team === 'village').length;
  
  const isOver = phase.includes('ende') || phase.includes('gewonnen') || 
                 wolfCount === 0 || villageCount === 0 || wolfCount >= villageCount;
...
async function executeWerewolfPhase(
  ...
  for (const wolf of livingWolves) {
    try {
      const targetCard = wolf.page.locator(`.spieler-card:has-text("${victim.name}")`);
      if (await targetCard.isVisible({ timeout: 2000 })) {
        await targetCard.click();
        await new Promise(resolve => setTimeout(resolve, ACTION_DELAY));
        
        const attackBtn = wolf.page.locator('button:has-text("Angreifen"), button:has-text("Wählen"), button:has-text("Bestätigen")');
...
async function executeDayVoting(
  ...
  if (livingWolves.length > 0) {
    ...
    voteTarget = livingPlayers[Math.floor(Math.random() * livingPlayers.length)];
  }
  ...
  for (let batch = 0; batch < voteBatches; batch++) {
    ...
    const votePromises = batchPlayers.map(async (player) => {
      try {
        const targetCard = player.page.locator(`.spieler-card:has-text("${voteTarget.name}")`);
        if (await targetCard.isVisible({ timeout: 2000 })) {
          await targetCard.click();
          await new Promise(resolve => setTimeout(resolve, ACTION_DELAY));
          
          const voteBtn = player.page.locator('button:has-text("Abstimmen"), button:has-text("Wählen")');
...
async function waitForNarratorAudio(page: Page): Promise<void> {
  try {
    ...
    const audioPlaying = page.locator('audio[autoplay], .erzaehlung-overlay, .notification');
 ...
 */
async function sendChatMessage(player: MassPlayer, message: string): Promise<void> {
  try {
    const chatInput = player.page.locator('#chat-input');
    const sendBtn = player.page.locator('button:has-text("paper-plane"), button[onclick="sendChat()"]');
...
async function executeRealDiscussion(players: MassPlayer[], durationMs: number = 10000): Promise<void> {
  ...
  for (let i = 0; i < numMessages; i++) {
    ...
    let message = DISCUSSION_MESSAGES[Math.floor(Math.random() * DISCUSSION_MESSAGES.length)];
 ...
 */
async function selectPlayerInUI(actor: MassPlayer, targetName: string): Promise<boolean> {
  try {
    const targetCard = actor.page.locator(`.spieler-card:has-text("${targetName}")`).first();
 ...
 */
async function clickActionButton(player: MassPlayer, ...buttonTexts: string[]): Promise<boolean> {
...
async function executeFullNightPhase(
  ...
  if (witch) {
    ...
    if (Math.random() > 0.8) {
      const poisonCandidates = allPlayers.filter(p => p.isAlive && p.id !== witch.id && p.name !== werewolfVictim);
      if (poisonCandidates.length > 0) {
        const poisoned = poisonCandidates[Math.floor(Math.random() * poisonCandidates.length)];
  ...
  if (werewolfVictim && werewolfVictim !== healed && werewolfVictim !== protectedPlayer) {
  ...
  for (const deadName of [...dead]) {
    ...
    if (deadPlayer?.isLover) {
      const otherLover = allPlayers.find(p => p.isLover && p.name !== deadName && p.isAlive);
      if (otherLover) {
...
async function executeFullDayPhase(
  ...
  if (deadLastNight.length > 0) {
    ...
    for (const deadName of deadLastNight) {
      ...
      if (deadPlayer?.role?.toLowerCase().includes('jäger')) {
        ...
        if (candidates.length > 0) {
          const hunterVictim = candidates[Math.floor(Math.random() * candidates.length)];
  ...
  for (const player of alivePlayers) {
    ...
    if (player.team === 'wolf') {
      ...
      voteTarget = villagerCandidates.length > 0 
        ? villagerCandidates[Math.floor(Math.random() * villagerCandidates.length)]
    ...
    if (eliminatedPlayer) {
      ...
      if (eliminatedPlayer.isMayor) {
        ...
        if (candidates.length > 0) {
          ...
          logEvent('👑 NACHFOLGE', `${successor.name} wird neuer Bürgermeister`);
        }
      }
    }
  } else {
...
test.describe(`Massives Werwolf-Spiel - ${GAME_MODE.name}`, () => {
  ...
  test.afterEach(async () => {
    if (hostPlayer) {
      try { await hostPlayer.context.close(); } catch {}
    }
    await cleanupMassPlayers(massPlayers);
    massPlayers = [];
  });

  test(`Vollständiges Spiel mit ${PLAYER_COUNT} Spielern`, async ({ browser }) => {
    resetWindowPositions();
    ...
    logEvent('INFO', `Pausen zwischen Aktionen: ${ACTION_DELAY / 1000} Sekunden`);
      ...
      logEvent('JOINED', `${massPlayers.length + 1} Spieler in ${joinTime.toFixed(1)}s beigetreten`);
      ...
      await host.page.waitForFunction(
        (count) => {
          const playerItems = document.querySelectorAll('#spieler-liste .spieler-karte[data-spieler-id]');
      ...
      logEvent('LOBBY', `Alle ${expectedPlayerCount} Spieler in der Lobby sichtbar!`);
      ...
      logEvent('TEAMS', `${wolves.length} Wölfe, ${villagers.length} Dorfbewohner, ${solos.length} Solo`);
      ...
      while (!gameOver && round < MAX_ROUNDS) {
        ...
        logEvent('STATUS', `Phase: ${status.phase}, Lebend: ${status.aliveCount}, Wölfe: ${status.wolfCount}, Dorf: ${status.villageCount}`);
        ...
        } else if (currentPhase.includes('abstimmung') || currentPhase.includes('tag')) {
          // Tag-Abstimmung
          const eliminated = await executeDayVoting(massPlayers, wolves);
          if (eliminated) {
            const eliminatedPlayer = massPlayers.find(p => p.name === eliminated);
        ...
        if (newStatus.isGameOver || newStatus.wolfCount === 0 || newStatus.villageCount === 0) {
          gameOver = true;
          logEvent('GAME_OVER', `Gewinner: ${newStatus.winner || (newStatus.wolfCount === 0 ? 'Dorf' : 'Werwölfe')}`);
        }
      }
      ...
      logEvent('SUCCESS', `=== ${GAME_MODE.name.toUpperCase()} TEST ABGESCHLOSSEN ===`);
  });
});
...
test.describe.skip('500 Spieler Stress & Performance Tests', () => {
  ...
  test('Gleichzeitiger Join von 500 Spielern', async ({ browser }) => {
    ...
    const massPlayers = await createMassPlayers(browser, roomCode, PLAYER_COUNT - 1, 100);
    
    const joinTime = (Date.now() - startTime) / 1000;
    
    logEvent('PERF', `500 Spieler in ${joinTime.toFixed(1)} Sekunden beigetreten`);
    logEvent('PERF', `Durchschnitt: ${(joinTime / 500 * 1000).toFixed(0)}ms pro Spieler`);
  ...
  test('Server-Stabilität bei 500 aktiven Verbindungen', async ({ browser }) => {
    resetWindowPositions();
    ...
    const hostResponsive = await host.page.evaluate(() => document.readyState === 'complete');
    ...
    for (const player of samplePlayers) {
      try {
        const isResponsive = await player.page.evaluate(() => document.readyState === 'complete');
    ...
    logEvent('STABILITY', `${responsiveCount}/${sampleSize} Stichproben-Spieler noch verbunden`);

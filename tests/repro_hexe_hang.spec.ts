...
test("Hexe Phase bleibt nicht hängen bei Nichts tun", async () => {
  ...
  if (!hexe) {
    console.log(
      "Keine Hexe in dieser Runde, überspringe Test-Details (aber fahre fort)",
    );
    await browser.close();
    return;
  }
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    const weiterBtn = pages[0].locator(
      'button:has-text("Weiter"), button:has-text("Nächste Phase")',
    );

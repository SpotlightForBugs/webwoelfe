...
class TestRoleDistribution:
    ...
    def test_small_games(self, player_count):
        ...
        assert (
            total == player_count
        ), f"Total roles {total} != player count {player_count}"
        ...
        assert (
            rollen.get("Dorfbewohner", 0) >= 0
        ), "Dorfbewohner count can't be negative"

        # Werewolves should be ~20% (1 per 5 players)
        wolf_count = sum(
            v for k, v in rollen.items() if "Wolf" in k or "werwolf" in k.lower()
        )
        assert (
            wolf_count <= player_count * 0.35
        ), f"Too many wolves ({wolf_count}) for {player_count} players"
    ...
    def test_medium_games(self, player_count):
        ...
        assert (
            12 <= wolf_percent <= 28
        ), f"Wolf team is {wolf_percent:.1f}% - should be 12-28%"
    ...
    def test_large_games(self, player_count):
        ...
        assert (
            10 <= wolf_percent <= 25
        ), f"Wolf team is {wolf_percent:.1f}% - should be 10-25% for large games"

        # Should have scaling information roles
        assert (
            rollen.get("Seherin", 0) >= 5
        ), f"500+ player games need multiple seers, got {rollen.get('Seherin', 0)}"
        assert (
            rollen.get("Hexe", 0) >= 3
        ), f"500+ player games need multiple witches, got {rollen.get('Hexe', 0)}"
        assert (
            rollen.get("Jaeger", 0) >= 5
        ), f"500+ player games need multiple hunters, got {rollen.get('Jaeger', 0)}"
        ...
        assert (
            solo_percent <= 8
        ), f"Solo roles are {solo_percent:.1f}% - should be <= 8%"
    ...
    def test_500_player_game_detailed(self):
        ...
        print(f"{'='*60}")
        print(
            f"  Village to Wolf ratio: {village_count}:{wolf_count} (1:{wolf_count/village_count:.2f})"
        )
    ...
    def test_1000_player_game_detailed(self):
        ...
        assert (
            teams["dorf"] > teams["werwolf"] * 2
        ), "Village must outnumber wolves significantly"
        assert teams["werwolf"] >= 100, "1000 player game should have 100+ wolves"
        assert (
            teams["werwolf"] <= 250
        ), "1000 player game should have at most 250 wolves"
    ...
    def test_no_negative_dorfbewohner(self):
        ...
        for player_count in [5, 10, 20, 50, 100, 200, 500, 1000]:
            ...
            assert (
                dorf_count >= 0
            ), f"Negative Dorfbewohner ({dorf_count}) for {player_count} players"
    ...
    def test_consistent_results(self):
        ...
        for player_count in [100, 500, 1000]:
            ...
            assert (
                rollen1 == rollen2
            ), f"Inconsistent results for {player_count} players"
...
class TestBalanceRatios:
    ...
    def test_wolf_ratio_scales_down_for_large_games(self):
        ...
        assert (
            large_wolf_percent <= small_wolf_percent + 5
...
class TestPerformance:
    ...
    def test_large_calculation_time(self):
        ...
        assert (
            elapsed < 1.0
        ), f"Role calculation too slow: {elapsed:.2f}s for 100 iterations"

...
class TestHexeInteractions:
    ...
    def test_hexe_heal_action(self):
        ...
        result = hexe_role.on_nacht_aktion(hexe_spieler, opfer, kontext, aktion="heilen")
    ...
    def test_hexe_poison_action(self):
        ...
        result = hexe_role.on_nacht_aktion(hexe_spieler, target, kontext, aktion="vergiften")
        
        assert result is not None
        assert result.erfolg == True
        assert result.effekte.get("toeten") == target.id or "hexe_vergiften" in result.effekte
    ...
    def test_hexe_cannot_use_empty_potion(self):
        ...
        result = hexe_role.on_nacht_aktion(hexe_spieler, target, kontext, aktion="heilen")
...
class TestSeherinInteractions:
    ...
    def test_seherin_sees_werewolf(self):
        ...
        assert result.effekte.get("erkannt") or "böse" in str(result.nachricht).lower() or "werwolf" in str(result.nachricht).lower()
...
class TestHeilerInteractions:
    ...
    def test_heiler_can_protect(self):
        ...
        if result:
            ...
            assert result.multi_target_updates or "ist_beschuetzt" in str(result.effekte)
    ...
    def test_state_fields_defined(self):
        ...
        for role_name in roles_with_state:
            ...
            if role:
                ...
                assert isinstance(fields, list), f"{role_name} state_fields should return list"
...
class TestRoleRegistration:
    ...
    def test_all_roles_have_unique_ids(self):
        ...
        for role in RoleRegistry.get_all():
            ...
            if role_id in ids:
                pytest.fail(f"Duplicate ID {role_id}: {role.info.name} and {ids[role_id]}")
    ...
    def test_all_roles_have_team(self):
        ...
        for role in RoleRegistry.get_all():
            ...
            assert isinstance(role.info.team, Team), f"{role.info.name} team is not Team enum"
    ...
    def test_all_roles_have_priority(self):
        ...
        for role in RoleRegistry.get_all():
            assert role.info.prioritaet is not None, f"{role.info.name} missing priority"
            assert isinstance(role.info.prioritaet, int), f"{role.info.name} priority not int"

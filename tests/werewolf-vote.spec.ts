...
test.describe("Werewolf Voting Test", () => {
  ...
  test("3: 5 players - werewolves pick a victim at night", async ({
    browser,
  }) => {
    ...
    const werewolves = players.filter((p) =>
      p.role?.toLowerCase().includes("werwolf"),
    );
    const villagers = players.filter(
      (p) =>
        !p.role?.toLowerCase().includes("werwolf") &&
        !p.role?.toLowerCase().includes("erzaehler"),
    );

    logEvent("TEAM", `Werewolves: ${werewolves.map((w) => w.name).join(", ")}`);
    logEvent("TEAM", `Villagers: ${villagers.map((v) => v.name).join(", ")}`);
    console.log(
      `WEREWOLVES: ${werewolves.map((w) => w.name).join(", ")} (${werewolves.length})`,
    );
    console.log(
      `VILLAGERS: ${villagers.map((v) => v.name).join(", ")} (${villagers.length})`,
    );
    ...
    while (roundCount < maxRounds) {
      ...
      if (
        currentPhase.includes("gewonnen") ||
        currentPhase.includes("vorbei")
      ) {
      ...
      if (
        currentPhase.includes("tag_start") ||
        ...
        logEvent(
          "SUCCESS",
          "Werewolf night voting test completed successfully",
        );
        break;
      }
      ...
      if (currentPhase.includes("werwolf")) {
        console.log(
          `[ROUND ${roundCount}] Werewolf night phase - performing attack...`,
        );
        ...
        const target =
          villagers.length > 0
            ? villagers[0]
            : players.find((p) => !p.role?.toLowerCase().includes("werwolf"));
        if (target) {
      ...
      if (currentPhase.includes("seherin")) {
        ...
        const seer = players.find((p) =>
          p.role?.toLowerCase().includes("seherin"),
        );
        if (seer) {
          try {
            const skipBtn = seer.page.locator(
              'button:has-text("Ueberspringen"), button:has-text("Weiter")',
            );
      ...
      if (currentPhase.includes("hexe")) {
        ...
        const witch = players.find((p) =>
          p.role?.toLowerCase().includes("hexe"),
        );
        if (witch) {
          try {
            const skipBtn = witch.page.locator(
              'button:has-text("Ueberspringen"), button:has-text("Weiter"), button:has-text("Niemanden")',
            );
            if (await skipBtn.isVisible()) {
              await skipBtn.click();
              await delayAction();
      ...
      if (
        ...
        console.log(
          `[ROUND ${roundCount}] Auto-advancing through preliminary phase...`,
        );
